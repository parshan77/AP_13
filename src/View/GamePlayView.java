...
public class GamePlayView extends Application {
    ...
    public void start(Stage primaryStage) throws Exception {
        ...
        showBuyLabels(root);
        showMoneyLabel(root);
        viewLevelRequirement(primaryStage);
    ...
    Label[] required = new Label[4];
    Label[] received = new Label[4];




    public void viewLevelRequirement(Stage primaryStage){
        Group group = new Group();
        ...
        for (int i = 0; i < imageViews.length; i++) {
            imageViews[i].relocate(Screen.getPrimary().getVisualBounds().getWidth() - 320 + 70 * i, Screen.getPrimary().getVisualBounds().getHeight() - 120);
            group.getChildren().add(imageViews[i]);
        }
    ...
    public void showUnderBar(Group group, Stage primaryStage, int requiredCows,
        ...
        buildImageView(group, "File:Textures\\Labels\\underBar.png",
...
//        imageViews[3] = buildImageView(root, null, x * 0.945, y * 0.810, x * 0.03, y * 0.05, true);
        ...
        required[0] = buildLabel(group, null, x * 0.802, y * 0.890, Font.font(16), true,
                "-fx-font-weight: bold");
        required[1] = buildLabel(group, null, x * 0.855, y * 0.890, Font.font(16), true,
                "-fx-font-weight: bold");
        required[2] = buildLabel(group, null, x * 0.905, y * 0.890, Font.font(16), true,
                "-fx-font-weight: bold");
        required[3] = buildLabel(group, null, x * 0.955, y * 0.890, Font.font(16), true,
                "-fx-font-weight: bold");

        received[0] = buildLabel(group, null, x * 0.802, y * 0.86, Font.font(16), true,
                "-fx-font-weight: bold");
        received[1] = buildLabel(group, null, x * 0.855, y * 0.86, Font.font(16), true,
                "-fx-font-weight: bold");
        received[2] = buildLabel(group, null, x * 0.905, y * 0.86, Font.font(16), true,
                "-fx-font-weight: bold");
        received[3] = buildLabel(group, null, x * 0.955, y * 0.86, Font.font(16), true,
                "-fx-font-weight: bold");
